...
## status.sh
For checking the status during the data collection, it displays CPU and GPU temrature, GPS location, lists all of the active screens from screen, displays kismet status, lists all of the recently modified files in the WiFi hashes this makes it no need to switch to the screens with the hcxdumptool to see the status.
