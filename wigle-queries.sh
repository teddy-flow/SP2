...
do
    ...
    curl -X GET "https://api.wigle.net/api/v2/network/search?onlymine=false&freenet=false&paynet=false&netid="$1"%3A"$2"%3A"$3"%3A"$4"%3A"$5"%3A"$test"" -H  "accept: application/json" -H  "API-KEY" > temp.txt
    cat 'temp.txt'
    temp="temp.txt"
